...
class ProductController extends Controller
{
    ...
    public function details_product($product_id, Request $request){
        $lay_mau = Product_details::select('color_name',)->where('product_id',$product_id)->distinct()->get();
        $lay_size = Product_details::select('size_name',)->where('product_id',$product_id)->orderBy('size_name','asc')->distinct()->get();
        $tam_mau=[];
