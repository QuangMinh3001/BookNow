...
class HomeController extends Controller
{
    public function index(){
        ...
        if(isset($_GET['sort_by'])){
            ...
            if($sort_by =='giam_dan'){
                $all_product = Product::where('product_status','1')->orderBy('product_price', 'DESC')->paginate(20)->appends (request()->query());
            }elseif($sort_by =='tang_dan'){
                $all_product = Product::where('product_status','1')->orderBy('product_price', 'ASC')->paginate(20)->appends (request()->query());
            }elseif($sort_by =='a_z'){
            ...
            $all_product = Product::where('product_status','1')->orderBy('product_id','desc')->limit(50)->paginate(9);
        }
    ...
    public function quen_mat_khau(){
        ...
        $brand_product =Brand::where('brand_status','1')->orderBy('brand_id','desc')->get();
        $ma_coupon = Coupon::where('coupon_qty','>','0')->orderBy('coupon_id','desc')->get();
        return view('pages.checkout.forget_password')->with('category',$cate_product)->with('brand',$brand_product)->with('coupon',$ma_coupon);
    }
    ...
    public function reset_new_pass(Request $request){
        ...
        if($count>0){
            ...
            return redirect('login-checkout')->with('message', 'Mật khẩu đã cập nhật mới.');
                ...
                return redirect()->back()->with('error', 'Vui lòng kiểm tra lại.');
            }
